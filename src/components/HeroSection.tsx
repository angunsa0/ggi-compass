...
export const HeroSection = () => {
  ...
  return (
    <section id="hero" className="relative min-h-screen">
      {/* Hero Background - Modern classroom image with overlay */}
      <div 
        className="absolute inset-0 bg-cover bg-center" 
        style={{ backgroundImage: `url(${heroClassroom})` }} 
      />
      {/* IKEA Blue overlay for readability */}
      <div className="absolute inset-0 bg-gradient-to-r from-primary/95 via-primary/80 to-primary/40" />
      
      {/* Hero Content */}
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-40 pb-20">
        <div className="max-w-3xl">
          <div className="inline-block px-4 py-1.5 bg-white/20 text-white text-xs font-bold rounded-full mb-8 uppercase tracking-widest border border-white/30">
          ...
          <p className="text-xl md:text-2xl text-white/80 mb-10 leading-relaxed">
            ...
            <span className="text-accent font-semibold">주식회사 지지아이</span>가 만드는 미래 교육의 기반입니다.
          </p>
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <Button 
              size="lg" 
              variant="outline" 
              className="border-white/50 text-white hover:bg-white/10 font-bold px-8 py-6 text-base" 
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pb-20">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {features.map((feature, index) => (
            <div 
              key={feature.title} 
              className="bg-white/90 backdrop-blur-sm border border-white/50 rounded-xl p-6 hover:shadow-lg transition-all group animate-fade-in" 
